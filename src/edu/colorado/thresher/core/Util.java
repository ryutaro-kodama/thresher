...
public class Util {
  ...
  */
  ...
  public static <T> Set<T> flattenAndIntersect(Collection<Set<T>> sets) {
    Set<T> flatSet = HashSetFactory.make();
    for (Set<T> set : sets) {
      if (flatSet.isEmpty()) flatSet.addAll(set);
      else flatSet.retainAll(set);
    }
    return flatSet;
  }
...
//        System.err.println("weird no producers for " + curEdge);
  ...
  public static Set<CGNode> getRelevantNodesForVar(PointerVariable var, HeapGraph hg) {
  ...
  /**
   * @return - get nodes where a write to var.field can occur
   */
  public static Set<CGNode> getRelevantForFieldWrite(PointerVariable var, IField field, HeapGraph hg) {
    Set<CGNode> nodes = getRelevantNodesForVar(var, hg);
    if (field != null) {
