...
//    Util.Assert(query instanceof CombinedPathAndPointsToQuery, "found bad query type " + query.getClass());
  ...
  private List<IPathInfo> enterCall(SSAInvokeInstruction instr, CallGraph cg, CGNode callee, boolean skip) {
    ...
    // TMP!
    if (calleeName == null || calleeName.contains("java/io") || calleeName.contains("StringBuffer, append")) {
      return IPathInfo.FEASIBLE;
    }
        
    // if this call is relevant
    if (skip || callee.getIR() == null || calleeName.contains("equals") || 
        calleeName.contains("hashCode") || calleeName.contains("indexOf") || calleeName.contains("String, startsWith") ||
        calleeName.contains("Iterator") || calleeName.contains("MessageQueue, next") || !isCallRelevantToQuery(instr, callee, cg)) { 
